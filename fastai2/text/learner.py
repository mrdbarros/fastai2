...
__all__ = ['match_embeds', 'load_ignore_keys', 'TextLearner', 'decode_spec_tokens', 'LMLearner',
           'language_model_learner', 'text_classifier_learner', 'text_regression_learner']
...
def text_regression_learner(dbunch, arch, seq_len=72, config=None, pretrained=True, drop_mult=0.5,
    ...
    meta = _model_meta[arch]
    learn = TextLearner(dbunch, model, loss_func=nn.MSELoss(), splitter=meta['split_clas'], **kwargs)
    if pretrained:
        if 'url' not in meta:
            warn("There are no pretrained weights for that architecture yet!")
            return learn
